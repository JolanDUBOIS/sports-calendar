...
def item_card(selection_name: str, item: SelectionItem):
    ...
    def on_add_filter_click():
        ...
        new_filter = SelectionService.add_empty_filter(item.uid)
        ...
        def on_confirm(**filter_updates):
            ...
            SelectionService.replace_filter(filter=updated_filter)
    ...
    def on_delete_click():
        ...
        Modal(title='Are you sure you want to delete this item?', confirm_color='red', reload_on_confirm=True).open(
            lambda: SelectionService.remove_item(item.uid)
        )

    with ui.expansion(value=is_open).classes('w-full bg-white rounded shadow') as exp:

        # HEADER
        with exp.add_slot('header'), ui.row().classes('w-full items-center justify-between'):
