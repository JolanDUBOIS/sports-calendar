...
from sports_calendar.core.selection import SelectionFilter, SelectionItem
...
def filter_block(filter: SelectionFilter, **kwargs):
    ...
    def on_delete_click():
        ...
        Modal('Are you sure you want to delete this filter?', confirm_color='red', reload_on_confirm=True).open(
            lambda: SelectionService.remove_filter(filter_uid=filter.uid)
        )

    with ui.card().classes('w-full p-2'), ui.row().classes('w-full items-center justify-between'):
        with ui.column():
            filter_body(filter)
