...
class CreateRegistry(Processor):
    """ TODO """

    def process(self, sources: dict[str, pd.DataFrame], output_key: str, threshold: float = 50, **kwargs) -> dict[str, pd.DataFrame]:
        """ TODO """
        ...
        registry = self.create_similarity_table(
            sources=sources,
            registry_parameters=registry_parameters,
            generic_tokens=generic_tokens
        )
    ...
    def create_similarity_table(
        ...
        for pair in pairs:
            ...
            pair_registry = self._create_similarity_table(
                dfA,
                dfB,
    ...
    @staticmethod
    def _validate_params(sources: dict[str, pd.DataFrame], registry_parameters: dict[str, dict], generic_tokens: list[str]) -> None:
        """ TODO """
