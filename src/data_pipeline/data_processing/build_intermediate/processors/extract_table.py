...
class ExtractTable(Processor):
    """ Extract table from another table in a simple process """
    ...
    extraction_instructions = {
        ...
        "espn_competitions": ["competition_id", "competition_name", "competition_abbreviation", "competition_midsizeName", "competition_slug", "source", "source_type"],
        "football_data_competitions": ["competition_id", "competition_name", "competition_code", "competition_type", "source", "source_type"],
    ...
    def process(self, data: pd.DataFrame, key: str, **kwargs) -> pd.DataFrame:
        ...
        if isinstance(instructions, list):
            return self._simple_extraction(data, instructions, **kwargs)
        elif isinstance(instructions, dict):
            return self._double_extraction(data, instructions, **kwargs)

    @staticmethod
    def _simple_extraction(data: pd.DataFrame, columns: list[str], deduplicate: bool = False) -> pd.DataFrame:
        """ TODO """
    ...
    @staticmethod
    def _double_extraction(data: pd.DataFrame, columns_mapping: dict[str, list[str]], deduplicate: bool = False) -> pd.DataFrame:
        """ TODO """
