...
from src.data_pipeline.data_processing.ingestion_landing import ingestion_landing
from src.data_pipeline.data_processing.build_intermediate import build_intermediate
...
def test(no: int):
    ...
    if no == 1:
        db_repo = Path(__file__).parent.parent / "data/repository"
        instructions_path = Path(__file__).parent.parent / "config/pipeline_config/daily/workflows/ingestion_landing.yml"
        instructions = read_yml_file(instructions_path)
        ingestion_landing(db_repo, instructions, manual=True)
        
    elif no == 2:
        db_repo = Path(__file__).parent.parent / "data/repository"
        instructions_path = Path(__file__).parent.parent / "config/pipeline_config/daily/workflows/build_intermediate.yml"
        instructions = read_yml_file(instructions_path)
        build_intermediate(db_repo, instructions, manual=True)
